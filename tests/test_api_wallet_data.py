    ...
    def test_create_wallet_data(self):
        with self.app.test_request_context():
            ...
            dta = {
                ...
                "effective": "2017-8-10",
                "nav": 9000,
                "invested_value": 4567.89
            }
            rv = self.create_resource('wallet_data', dta)
            data = json.loads(rv.data)
            self.assertEqual(data["wallet_id"], 1)

    def test_get_wallet_data(self):
        with self.app.test_request_context():
            create_wallet_data(self.client)
            resp = self.get_resource('wallet_data', resource_id=1)
            data = json.loads(resp.data)
            self.assertEqual(data['wallet_id'], 1)

    def test_get_multiple_wallet_data(self):
        with self.app.test_request_context():
            create_wallet_data(self.client)
            resp = self.get_resource('wallet_data')
            data = json.loads(resp.data)
            self.assertEqual(len(data), 4)
            self.assertEqual(data[0]['wallet_id'], 1)

    def test_missing_wallet_data(self):
        with self.app.test_request_context():
            resp = self.get_resource('wallet_data', resource_id=1)
